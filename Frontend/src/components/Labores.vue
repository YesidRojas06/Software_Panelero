...
/>
      ...
      <q-dialog v-model="nuevo" persistent transition-show="scale" transition-hide="scale">
        <q-card class="bg-teal text-dark"  style="width: 500px; max-width: 80vw;">
          ...
          <p v-if="bd == 0 ">pago_diario</p>
          <input v-if="bd == 0 " type="text" v-model="pago_diario" class="form-input">
...
if (!area_realizar.value.includes("@")) {
  q.notify({
    type: "negative",
    message: "El area_realizar electrónico debe contener el símbolo @",
    position: "top",
  });
}
}
