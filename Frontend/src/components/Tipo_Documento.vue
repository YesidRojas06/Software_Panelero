      ...
      <q-dialog v-model="nuevo" persistent transition-show="scale" transition-hide="scale">
        <q-card class="bg-teal text-dark" style="width: 500px; max-width: 80vw;">
          ...
          <q-card-section style="background-color:rgb(14, 224, 14);">
            <div align="center" class="text-h6">{{ bd === 0 ? "Guardar Tipo_Documento" : "Editar Tipo_Documento" }}</div>
          </q-card-section>
  ...
  async function guardarEditarDatos() {
    if (bd.value === 0) {
      ...
      if (siglas.value === "") {
        q.notify({
          type: "negative",
          message: "Por favor, ingrese un siglas validas.",
          position: "top",
        });
        return;
      }
      ...
      if (siglas.value === "") {
        q.notify({
          type: "negative",
          message: "Por favor, ingrese un siglas válido.",
          position: "top",
        });
        return;
      }
