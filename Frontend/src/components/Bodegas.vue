...
function guardar() {
  ...
  postUser.addBodegas(nuevoUsuario).then(() => {
    listarUsuarios();
  });
}
async function inactive(props) {
  console.log("desactivado", props.Codigo);
  let res = await getBodegas.inactiveUser(props._id);
  listarUsuarios();
}

async function active(props) {
  console.log("activado", props.Codigo);
  let res = await getBodegas.activeUser(props._id);
  listarUsuarios();
}
...
async function guardarEditarDatos() {
  if (bd.value == 0) {
    ...
    let res = await getBodegas.addBodegas(datos);
    listarUsuarios();
    ...
    let res = await getBodegas.editUser(_id, datos);
    listarUsuarios();
    console.log(res);
    q.notify({
      type: "positive",
