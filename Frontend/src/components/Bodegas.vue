      ...
      <q-card class="bg-teal text-dark" style="width: 500px; max-width: 80vw">
        <q-card-section style="background-color: rgb(14, 224, 14)">
          <div align="center" class="text-h6">
            {{ bd == 0 ? "Guardar Ususario" : "Editar Usuario" }}
          </div>
        </q-card-section>
        ...
        <q-card-section class="q-pt-none">
          ...
          <input type="text" v-model="Numero_De_Bodega" class="form-input" />
...
let columns = ref([
  ...
  {
    name: "Numero De Bodegas",
    required: true,
    label: "Numero De Bodegas",
    align: "left",
    field: "Numero De Bodegas",
    sortable: false,
  },
...
function edit(row) {
  ...
  rol.value = row.rol;
  Numero_De_Bodega.value = row.Numero_De_Bodega;
}
...
function guardar() {
  ...
  const nuevoUsuario = {
    ...
    correo: correo.value,
    rol: rol.value,
    Numero_De_Bodega: Numero_De_Bodega.value,
  };
